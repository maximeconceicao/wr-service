...
seneca.client({      // ce module enverra les messages au service stats
    ...
    pin: 'role:stats,cmd:*' // stats attend les messages...
})
...
var wr = function (options) {
    ...
    this.add('role:wr,cmd:create', function (msg, done) {
        ...
        seneca.act({ role: 'stats', cmd: 'created', applicant: msg.applicant });
    })
        ...
        this.make('wr').load$(msg.id, function (err, wr) {
            ...
            if (wr !== null) {
                if (wr.state === states.CLOSED) {
                    ...
                    if (msg.state === states.CLOSED) seneca.act({ role: 'stats', cmd: 'closed', applicant: wr.applicant });
            ...
            this.make('wr').list$(function (err, list) {
                ...
                list.forEach(function (wr) {
                    if (wr.state !== states.CLOSED) {
                        ...
                        seneca.act({ role: 'stats', cmd: 'deleted', applicant: wr.applicant });
                    }
                });
